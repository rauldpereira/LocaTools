    ...
    try {
        ...
        const ordensCriadas = await sequelize.transaction(async (t) => {
            ...
            for (const item of itens) {
                ...
                if (!gruposPorData[startStr]) {
                    gruposPorData[startStr] = {
                ...
                gruposPorData[startStr].itens.push(item);
            }

            const numeroDeViagens = Object.keys(gruposPorData).length;

            // Define o Frete POR VIAGEM 
            let fretePorViagem = 0;
            if (tipo_entrega === 'entrega') {
                if (valor_frete && Number(valor_frete) > 0) {
                    fretePorViagem = Number(valor_frete) / numeroDeViagens;
                } else if (endereco_entrega) {
                    fretePorViagem = await calcularFreteInterno(endereco_entrega);
                }
            }

            const ordens = [];

            // Processar CADA data como um Pedido Independente
            for (const [dataSaida, grupo] of Object.entries(gruposPorData)) {
                
                let data_fim_grupo = dataSaida;
                let subtotal_grupo = 0;
                const unidadesParaAlugar = {}; 

                // Valida e calcula os itens APENAS desse dia
                ...
                ordens.push(ordemDeServico);
            }

            return ordens;
        });

        // Retorna o ID do primeiro pedido para o frontend redirecionar sem quebrar, mas manda a lista completa em 'ids'
        res.status(201).json({ 
            message: 'Pedidos criados com sucesso!', 
            id: ordensCriadas[0].id,
            ids: ordensCriadas.map(o => o.id) 
        });
...
const generateContract = async (req, res) => {
    ...
    try {
        ...
        const clausulas = [
            ...
            `e) O LOCATÁRIO poderá solicitar o reagendamento das datas de locação, sujeito à disponibilidade. Esta solicitação deve ser realizada ÚNICA E EXCLUSIVAMENTE através do sistema da LocaTools. ${configLoja?.taxa_reagendamento && Number(configLoja.taxa_reagendamento) > 0 ? 'Será cobrada uma taxa administrativa de R$ ' + Number(configLoja.taxa_reagendamento).toFixed(2) + ' por cada alteração solicitada após a confirmação deste contrato.' : 'O reagendamento está sujeito a análise e possíveis taxas administrativas vigentes no momento da solicitação.'}`,
            'f) A prorrogação deste contrato deve ser realizada EXCLUSIVAMENTE através do site/plataforma. Caso o equipamento não seja devolvido na data prevista e não haja renovação formalizada, será cobrada multa por atraso conforme política vigente;',
            'g) O equipamento deverá ser devolvido na loja física ou, caso deseje retirada pela equipe, a solicitação deve ser feita obrigatoriamente através do site para geração de protocolo e agendamento;',
            'h) Ter ciência do tratamento dos dados pessoais fornecidos para realização do presente CONTRATO, nos termos da LGPD;',
            'i) Devolver o bem nas condições estabelecidas, no fim do contrato — caso o locatário provocar danos ao equipamento ou não devolver em condições favoráveis, será cobrada uma multa;',
            '',
