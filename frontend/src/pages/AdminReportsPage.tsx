...
const AdminReportsPage: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    "financeiro" | "ocorrencias" | "inventario"
  >("financeiro");
  const [loading, setLoading] = useState(false);

  const [bottomView, setBottomView] = useState<
    "extrato" | "equipamentos" | "abandonos"
  >("extrato");

  const [financialData, setFinancialData] =
    useState<ReportDataFinancial | null>(null);
  const [operationalData, setOperationalData] =
    useState<ReportDataOperational | null>(null);
  ...
  useEffect(() => {
    ...
    if (activeTab === "ocorrencias" || activeTab === "inventario")
      fetchOperational();
    setSearchTerm("");
    setSortConfig(null);
  }, [activeTab, token]);
  ...
  const handleExportPDF = () => {
    ...
    const dadosParaExportar = processTableData(financialData.extrato, [
      "descricao",
      "tipo",
    ]);

    const tableRows = dadosParaExportar.map((item) => {
      const valorFmt = item.valor.toLocaleString("pt-BR", {
        style: "currency",
        currency: "BRL",
      });
      return [
  ...
  const getEvolutionData = () => {
    ...
    const map = new Map();
    financialData.history.receitas.forEach((r: any) =>
      map.set(r.mes, { name: r.mes, receita: Number(r.total), prejuizo: 0 }),
    );
    financialData.history.prejuizos.forEach((p: any) => {
      const current = map.get(p.mes) || {
        name: p.mes,
        receita: 0,
        prejuizo: 0,
      };
    ...
    >
      ...
      >
        <button
          ...
          style={activeTab === "financeiro" ? activeTabStyle : tabStyle}
        >
          📊 Financeiro
        </button>
        <button
          onClick={() => setActiveTab("ocorrencias")}
          style={activeTab === "ocorrencias" ? activeTabStyle : tabStyle}
        >
          🚨 B.O.s e Roubos
        </button>
        <button
          onClick={() => setActiveTab("inventario")}
          style={activeTab === "inventario" ? activeTabStyle : tabStyle}
        >
          📦 Inventário
        </button>
      </div>
      ...
      {activeTab === "financeiro" && financialData && (
        <div>
          {financialData.inventory.total > 0 &&
            financialData.inventory.manutencao /
              financialData.inventory.total >=
              0.5 && (
              <div
                style={{
              ...
              >
                ...
                <div>
                  ...
                  <p style={{ margin: 0, color: "#333", fontSize: "1.1rem" }}>
                    <strong>{financialData.inventory.manutencao}</strong> de{" "}
                    <strong>{financialData.inventory.total}</strong>{" "}
                    equipamentos (
                    <strong>
                      {(
                        (financialData.inventory.manutencao /
                    ...
                    do seu estoque total) estão inoperantes ou em manutenção.{" "}
                    <br />
                    <span style={{ color: "#d32f2f", fontSize: "0.95rem" }}>
                      Isso está travando seu potencial de faturamento. Verifique
                      os B.O.s urgentemente!
                    </span>
                  </p>
                </div>
              </div>
          ...
          >
            <KpiCard
              ...
              value={`R$ ${financialData.kpis.faturamentoTotal.toFixed(2)}`}
              icon="💰"
              color="#28a745"
            />
            <KpiCard
              title="Prejuízo (Perdas)"
              value={`R$ ${financialData.kpis.totalPrejuizoAberto.toFixed(2)}`}
              icon="📉"
              color="#dc3545"
            />
            <KpiCard
              title="Lucro Líquido Real"
              value={`R$ ${financialData.kpis.lucroLiquido.toFixed(2)}`}
              icon="📊"
              color={
                financialData.kpis.lucroLiquido >= 0 ? "#28a745" : "#dc3545"
              }
              highlight
            />
            <KpiCard
              title="Recuperado"
              value={`R$ ${financialData.kpis.totalPrejuizoRecuperado.toFixed(2)}`}
              icon="🤝"
              color="#17a2b8"
            />
            <KpiCard
                ...
                <ResponsiveContainer>
                  <PieChart>
                    <Pie
                      ...
                      paddingAngle={5}
                      dataKey="value"
                      label={({ payload }: any) => payload.name}
                    >
                  ...
                  >
                    <span>
                      ...
                      <span style={{ color: "#888", fontSize: "0.85rem" }}>
                        ({equip.alugueis} locações)
                      </span>
                    </span>
                    <strong style={{ color: "#28a745" }}>
                      R$ {equip.receita.toFixed(2)}
                    </strong>
                  </li>
                ))}
                {financialData.topEquipamentos.length === 0 && (
                  <p style={{ color: "#888" }}>Sem dados no período.</p>
                )}
              </ul>
              <button
                onClick={() => {
                  setBottomView("equipamentos");
                  document
                    .getElementById("detalhes-view")
                ...
                {financialData.topAbandonados.slice(0, 5).map((equip, i) => (
                  ...
                  >
                    ...
                    <strong style={{ color: "#fd7e14" }}>
                      {equip.quantidade} desistências
                    </strong>
                  </li>
                ))}
                {financialData.topAbandonados.length === 0 && (
                  <p style={{ color: "#888" }}>
          ...
          >
            <button
              onClick={() => setBottomView("extrato")}
              style={bottomView === "extrato" ? activeTabStyle : tabStyle}
            >
              📜 Extrato Financeiro
            </button>
            <button
              onClick={() => setBottomView("equipamentos")}
              style={bottomView === "equipamentos" ? activeTabStyle : tabStyle}
            >
              🏆 Tabela de Lucro{" "}
            </button>
            <button
              onClick={() => setBottomView("abandonos")}
              style={bottomView === "abandonos" ? activeTabStyle : tabStyle}
            >
              ⚠️ Tabela de Abandonos
            </button>
          </div>
          ...
          {bottomView === "extrato" && (
            <div style={cardContainerStyle}>
              ...
              >
                <h3
                  style={{ ...cardTitleStyle, borderBottom: "none", margin: 0 }}
                >
                  📜 Extrato Detalhado
                </h3>
              ...
              >
                ...
                <tbody>
                  {processTableData(financialData.extrato, [
                    "descricao",
                    "tipo",
                  ]).map((item: any, idx) => (
                    <tr
          ...
          {bottomView === "equipamentos" && (
            <div style={cardContainerStyle}>
              ...
              >
                ...
                <tbody>
                  {processTableData(financialData.topEquipamentos, [
                    "nome",
                  ]).map((equip: any, idx) => (
            ...
            <div style={cardContainerStyle}>
              ...
              >
                <h3
                  style={{ ...cardTitleStyle, borderBottom: "none", margin: 0 }}
                >
                  ⚠️ Ranking Completo de Abandonos
                </h3>
                <input
                  type="text"
                  placeholder="Buscar máquina..."
              ...
              >
                <thead style={{ backgroundColor: "#fff3cd" }}>
                  <tr>
                    <SortableTh label="Equipamento" sortKey="nome" />
                    <SortableTh
                      label="Desistências no Checkout"
                      sortKey="quantidade"
                    />
                  </tr>
                </thead>
                <tbody>
                  {processTableData(financialData.topAbandonados, ["nome"]).map(
                    (equip: any, idx) => (
                      <tr key={idx} style={{ borderBottom: "1px solid #eee" }}>
                        <td style={{ ...tdStyle, fontWeight: "bold" }}>
                          {equip.nome}
            ...
            <tbody>
              {processTableData(operationalData.ocorrencias, [
                ...
                <tr
                  ...
                  style={{
                    borderBottom: "1px solid #eee",
                    backgroundColor: bo.resolvido ? "#f9fff9" : "#fff5f5",
                  }}
                >
                  <td style={tdStyle}>
                    {new Date(bo.data).toLocaleDateString()}
                  </td>
                  <td
                    style={{
                      ...tdStyle,
                      fontWeight: "bold",
                      color: bo.tipo === "ROUBO" ? "red" : "orange",
                    }}
                  >
                    {bo.tipo}
                  </td>
                  ...
                  <td style={{ ...tdStyle, fontWeight: "bold" }}>
                    R$ {Number(bo.valor).toFixed(2)}
                  </td>
                  <td style={tdStyle}>
                    {bo.resolvido ? (
                      <span style={{ color: "green" }}>RESOLVIDO ✅</span>
                    ) : (
                      <span style={{ color: "red" }}>PENDENTE ⚠️</span>
                    )}
                  </td>
                  <td
                    style={{
      ...
      {activeTab === "inventario" && operationalData && (
        <div style={cardContainerStyle}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {processTableData(operationalData.inventario, [
                "equipamento",
                "status",
                "id",
                "codigo_serial",
                ...
                <tr key={item.id} style={{ borderBottom: "1px solid #eee" }}>
                  ...
                  <td
                    style={{
                      ...tdStyle,
                      color: item.codigo_serial ? "#333" : "#999",
                      fontWeight: item.codigo_serial ? "bold" : "normal",
                    }}
                  >
...
const KpiCard: React.FC<{
  ...
  >
    ...
    <div>
      ...
      <span style={{ fontSize: "2rem", fontWeight: "bold", color: "#2c3e50" }}>
        {value}
      </span>
    </div>
  </div>
